...
const NavigationBar = () => {
  const onPress = (tab) => console.log('Pressed tab:', tab);

  return (
    <View style={styles.navBar}>
      {['Home', 'Schedule', 'Map', 'Profile'].map((tab, index) => (
        <TouchableOpacity key={index} style={styles.navItem} onPress={() => onPress(tab)}>
          <Text style={styles.navIcon}>{tab === 'Home' ? '🏠' : tab === 'Schedule' ? '🗓️' : tab === 'Map' ? '🗺️' : '👤'}</Text>
              ...
              <View key={stop.id} style={styles.timelineItem}>
                ...
                <View style={styles.timelineRight}>
                  <View style={styles.stopHeader}>
                    <Text style={[styles.stopTitle, stop.status === 'Upcoming' && styles.upcomingText, stop.status === 'Completed' && styles.completedText]}>
                      {stop.name}
                    </Text>
                    <View style={[styles.statusBadge, stop.status === 'Completed' && styles.completedBadge, stop.status === 'Current' && styles.currentBadge, stop.status === 'Next' && styles.nextBadge, stop.status === 'Upcoming' && styles.upcomingBadge]}>
                      <Text style={[styles.statusBadgeText, stop.status === 'Upcoming' && styles.upcomingBadgeText]}>{stop.status}</Text>
                    </View>
                  </View>
                  <Text style={[styles.timeLabel, stop.status === 'Upcoming' && styles.upcomingTimeLabel, stop.status === 'Current' && styles.currentTimeLabel, stop.status === 'Next' && styles.nextTimeLabel, stop.status === 'Completed' && styles.completedTimeLabel]}>
                    {stop.timeLabel}
                  </Text>
                </View>
              </View>
...
const styles = StyleSheet.create({
  ...
  bannerRoute: { color: 'white', fontSize: 16 },
  ...
  greyCircle: { width: 12, height: 12, borderRadius: 6, backgroundColor: '#bbb' },
  ...
  navBar: { position: 'absolute', bottom: 0, left: 0, right: 0, height: 70, backgroundColor: '#fff', borderTopWidth: 1, borderTopColor: '#ddd', flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', paddingBottom: 10 },
